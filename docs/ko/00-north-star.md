...
workflow를 설계하는 대신, 하나의 회사 인터페이스에 목표를 전달합니다. 회사는 acceptance와 위험 경계를 보존할
수 있는 가장 작은 구조인 **최소 충분 실행 구조**를 만들고, 근거와 결과를 통합해 보고합니다.
...
- chain-of-thought를 저장하거나 많은 raw log를 사람에게 읽을 설명으로 간주하지 않습니다.
...
이 개념은 반복 실패를 줄이면서도 조용한 자율성을 늘리지 않는지로 평가해야 합니다. 승인된 결과의 품질,
lower-tail·complete failure, 실패 후 수정 가능성, 구조 선택의 설명성, 불필요한 위임, 조정·사람 review burden,
예산 준수, 중요한 작업을 사용자가 검사·제한할 수 있는지가 핵심입니다. Terminal result는 사용자가 raw log를
재생하지 않아도 되는 짧고 evidence-bound한 결론을 남겨야 합니다. 더 빠른 출력만으로 더 나은 회사라고 볼 수는
없습니다.
