...
이 문서는 불완전한 정보 아래에서 작동하는 AI 시스템의 통제 모델을 정의합니다. Noruct는 자신감 있는 문장으로
판정 기준의 부재를 해결한다고 주장하지 않습니다. 근거, 추론, 가정, 권한 있는 결정, 실행 가능한 검사, 나중에
관측된 결과의 차이를 기록합니다.
...
| --- | --- | --- |
| 기록된 맥락의 부족 | 인용 가능한 자료를 찾고 빈 곳을 드러냄 | 기록되지 않은 암묵지를 복원한다고 주장 |
| 근거의 충돌 | 충돌·최신성·불확실성을 표시 | 근거 없이 승자를 선언 |
| 가치의 충돌 | 선택지와 결과를 정리 | 사용자의 선호를 소유 |
| 신뢰할 수 있는 검사 부재 | 주장을 좁히거나 사용자에게 올림 | 자기 검토를 독립 검증으로 포장 |
| 현실 결과의 지연 | 결과 기록과 재검토 시점 유지 | 작업 완료를 현실 성공으로 간주 |

이 표는 제품의 경계입니다. 지식과 검증이 회사를 더 낫게 만들 수는 있지만, 회사를 자율적인 도덕·법적 주체로
만들지는 않는다는 뜻입니다.
