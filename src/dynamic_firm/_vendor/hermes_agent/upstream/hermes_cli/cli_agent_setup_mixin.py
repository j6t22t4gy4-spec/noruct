...
class CLIAgentSetupMixin:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._session_db is None:
            try:
                # The Noruct fork keeps the upstream CLI surface but routes
                # durable session state through the Company ledger.
                from noruct_firm.session import open_company_session_db
                self._session_db = open_company_session_db()
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            reopen = getattr(self._session_db, "reopen_session", None)
            if callable(reopen):
                reopen(self.session_id)
        ...
        try:
            ...
            # Fork insertion seam: the CLI session contributes only a
            # read-only Company projection.  Provider credentials, tool
            # handlers, approvals and durable state remain owned by Noruct's
            # parent runtime.
            try:
                from noruct_firm import attach_company_context
                ...
                logger.debug("Noruct Company context hook unavailable", exc_info=True)
