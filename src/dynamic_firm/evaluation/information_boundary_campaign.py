...
from . import information_boundary_campaign_contracts as _campaign_contracts
...
def information_boundary_pair_status(
    directory: str | Path,
) -> InformationBoundaryPairStatus:
    with InformationBoundaryPairStore(directory) as store:
        _, manifest, preflight = _campaign_artifacts(store)
        events = store.events()
        root = store.directory
    slots = information_boundary_pair_expected_runs()
    ...
    qualified: dict[tuple[str, str], LiveInformationBoundaryRecord] = {}
    for event in events:
        if event.fixture is None or event.strategy is None:
            continue
        key = (event.fixture, event.strategy)
        if key not in slots:
            raise ValueError("Information-boundary pair ledger contains an unknown slot")
        if event.kind == CampaignEventKind.RUN_STARTED:
            if key in started:
                raise ValueError("Information-boundary pair reuses a run slot")
            started[key] = event
        elif event.kind == CampaignEventKind.RUN_RECORDED:
            if key not in started or key in recorded or key in failed:
                raise ValueError("Information-boundary pair record has no unique start")
            recorded[key] = event
            ...
            if (
                ...
                raise ValueError("Information-boundary pair failure has no unique start")
            if event.kind == CampaignEventKind.RUN_INTERRUPTED:
                interrupted_terminal[key] = event
            else:
                failed[key] = event
    for key, event in recorded.items():
        path = _sealed_path(root, event.payload.get("record_path"), "records-v5")
        if _sha256_file(path) != event.payload.get("record_file_sha256"):
            raise ValueError("Information-boundary pair sealed record changed")
        ...
        if (
            ...
            raise ValueError("Information-boundary pair ledger projection changed")
    for event in (*failed.values(), *interrupted_terminal.values()):
        path = _sealed_path(root, event.payload.get("failure_path"), "failures-v5")
        if _sha256_file(path) != event.payload.get("failure_file_sha256"):
            raise ValueError("Information-boundary pair sealed failure changed")
        _validate_failure(
            path,
            manifest,
    ...
    if solo is not None:
        if not solo.validation.passed:
            ...
            solo.artifact.quality_score
            > 1.0 - manifest.quality_gain_threshold + 1e-12
        ):
    ...
    if external_calls > manifest.max_model_calls_pair:
        raise ValueError("Information-boundary pair call budget changed")
    ...
    return InformationBoundaryPairStatus(
        schema_version=INFORMATION_BOUNDARY_PAIR_STATUS_SCHEMA,
...
async def prepare_information_boundary_pair(
    directory: str | Path,
    *,
    preflight: str | Path,
    wheel: str | Path,
...
) -> InformationBoundaryPairPreparation:
    target = Path(directory).expanduser().resolve()
    if target.exists() and (not target.is_dir() or any(target.iterdir())):
        raise ValueError(f"Information-boundary pair directory must be empty: {target}")
    phase44_path = Path(preflight).expanduser().resolve()
    ...
    report = phase44.get("report")
    offline_ready = (
        isinstance(report, dict)
        and report.get("passed") is True
        and report.get("ready_for_live_control_pair") is True
        and phase44.get("ready") is True
        and phase44.get("external_provider_calls") == 0
        and phase44.get("quota_consumed") is False
    )
    checks = (
        InformationBoundaryCheck(
            "phase44-provider-free-preflight",
            offline_ready,
            f"benchmark={phase44['benchmark_id']},provider-calls=0",
        ),
        InformationBoundaryCheck(
            "source-snapshot-frozen",
            source_revision == phase44["source_revision"],
            source_revision,
        ),
        InformationBoundaryCheck(
            "wheel-hash-frozen",
            distribution_sha256 == phase44["distribution_sha256"],
            distribution_sha256,
        ),
        InformationBoundaryCheck(
            "evaluation-contract-current",
            phase44["noruct_version"] == __version__
            and phase44["authority_profile"] == INFORMATION_BOUNDARY_AUTHORITY_PROFILE
            and phase44["memory_revision"] == information_boundary_memory_revision()
            and phase44["fixture_revision"] == information_boundary_fixture_revision()
            and phase44["benchmark_revision"]
            == information_boundary_benchmark_revision(),
            "version, authority, memory, fixture, and benchmark match",
        ),
        InformationBoundaryCheck(
        ...
        InformationBoundaryCheck(
            "exact-solo-then-admission-pair",
            tuple(item.strategy for item in manifest.expected_runs)
            == INFORMATION_BOUNDARY_LIVE_STRATEGIES
            and len({item.workload_hash for item in manifest.expected_runs}) == 1
            and len({item.run_id for item in manifest.expected_runs}) == 2,
    ...
    pair_preflight = InformationBoundaryPairPreflight(
        schema_version=INFORMATION_BOUNDARY_PAIR_PREFLIGHT_SCHEMA,
        benchmark_id=manifest.benchmark_id,
        recorded_at=utc_now().isoformat(),
        phase44_benchmark_id=manifest.preflight_benchmark_id,
        phase44_content_hash=manifest.preflight_content_hash,
    ...
    manifest_path = _write_private(
        target / "manifest-v5.json",
        json.dumps(to_primitive(manifest), ensure_ascii=False, sort_keys=True, indent=2),
    )
    pair_preflight_path = _write_private(
        target / "preflight-v5.json",
        json.dumps(
            to_primitive(pair_preflight),
            ensure_ascii=False,
            sort_keys=True,
            indent=2,
        ),
    )
    copied_phase44_path = _write_private(
        target / "phase44-preflight-v3.json",
        json.dumps(phase44, ensure_ascii=False, sort_keys=True, indent=2),
    )
    with InformationBoundaryPairStore(target, create=True) as store:
        store.initialize(
            {
                "schema_version": INFORMATION_BOUNDARY_PAIR_LEDGER_SCHEMA,
                "benchmark_id": manifest.benchmark_id,
                "manifest_content_hash": manifest.content_hash,
                "manifest_file_sha256": _sha256_file(manifest_path),
                "preflight_file_sha256": _sha256_file(pair_preflight_path),
                "phase44_file_sha256": _sha256_file(copied_phase44_path),
        ...
        store.append(
            ...
            payload={
                "ready": pair_preflight.ready,
                "external_model_calls": 0,
                "quota_consumed": False,
                "expected_runs": 2,
            },
        )
    return InformationBoundaryPairPreparation(
        pair_preflight,
        information_boundary_pair_status(target),
    )
...
def _verify_runtime_inputs(
    ...
    if (
        ...
        raise ValueError("Information-boundary pair source snapshot changed")
    if (
        wheel_distribution_sha256(Path(str(metadata["wheel_path"])))
        != manifest.distribution_sha256
    ):
        raise ValueError("Information-boundary pair wheel changed")
    if (
        information_boundary_memory_revision() != manifest.memory_revision
        or information_boundary_fixture_revision() != manifest.fixture_revision
        or information_boundary_benchmark_revision() != manifest.benchmark_revision
    ):
        raise ValueError("Information-boundary pair evaluation contract changed")


async def run_next_information_boundary_pair_slot(
    directory: str | Path,
    *,
    ...
    live_runner: Callable[
        ...,
        Awaitable[LiveInformationBoundaryRecord],
    ]
    | None = None,
) -> InformationBoundaryPairRunResult:
    status = information_boundary_pair_status(directory)
    if (
        ...
        raise ValueError(
            f"Information-boundary pair cannot run while state={status.state.value}"
        )
    ...
    with InformationBoundaryPairStore(directory) as store:
        metadata, manifest, _ = _campaign_artifacts(store)
    ...
    runner = live_runner or run_live_information_boundary_evaluation
    try:
        record = await runner(
            config,
        ...
        record_path = _write_private(
            ...
            live_information_boundary_record_to_json(record),
        )
        ...
        return InformationBoundaryPairRunResult(
            event=event,
            status=information_boundary_pair_status(directory),
        ...
        relative = Path("failures-v5") / (
            f"{start.sequence:02d}-{status.next_fixture}-{status.next_strategy}.json"
        )
        code = exc.code if isinstance(exc, ModelProviderError) else type(exc).__name__
        failure_payload = {
            "schema_version": INFORMATION_BOUNDARY_PAIR_FAILURE_SCHEMA,
...
def compare_information_boundary_pair(
    directory: str | Path,
    *,
    output_path: str | Path | None = None,
) -> InformationBoundaryPairComparison:
    status = information_boundary_pair_status(directory)
    if status.state != CampaignState.COMPLETE:
        raise ValueError(
            "Information-boundary comparison requires two sealed records; "
            f"state={status.state.value},completed={status.completed_runs}/2"
        )
    root = Path(directory).expanduser().resolve()
    with InformationBoundaryPairStore(root) as store:
        _, manifest, _ = _campaign_artifacts(store)
        events = store.events()
        recorded = {
        ...
        records = tuple(
            ...
            for slot in information_boundary_pair_expected_runs()
        )
        ...
        checks = (
            ...
            InformationBoundaryCheck(
                "exact-typed-organization-admission",
        ...
        safety_gate = checks[2].passed and checks[3].passed
        organization_gate = checks[4].passed
        budget_gate = checks[6].passed
        pair_gate = all(check.passed for check in checks)
        if not safety_gate:
            outcome = "SAFETY_GATE_FAILED"
            direction = "FREEZE_AND_FIX_INFORMATION_BOUNDARY"
        elif not organization_gate:
            outcome = "ADMISSION_FALSE_NEGATIVE"
            direction = "FIX_TYPED_ORGANIZATION_ADMISSION"
        elif not checks[5].passed:
            outcome = "ORGANIZATION_VALUE_NOT_OBSERVED"
            direction = "REVIEW_BOUNDARY_TASK_AND_RUNTIME"
        elif pair_gate:
            outcome = "INFORMATION_BOUNDARY_VALUE_OBSERVED"
            direction = "ADD_ONE_INDEPENDENT_INFORMATION_BOUNDARY_FIXTURE"
        else:
            outcome = "LIVE_PAIR_CONTRACT_FAILED"
            direction = "FREEZE_AND_FIX_LIVE_PAIR_CONTRACT"
        comparison = InformationBoundaryPairComparison(
            schema_version=INFORMATION_BOUNDARY_PAIR_COMPARISON_SCHEMA,
            benchmark_id=manifest.benchmark_id,
            manifest_content_hash=manifest.content_hash,
            completed_runs=2,
            expected_runs=2,
